...
module amm::interest_protocol_amm_tests {
  ...
  fun test_new_pool() {
    ...
    next_tx(test, alice);
    {
      ...
      assert_eq(interest_protocol_amm::lp_coin_supply<ETH, USDC, IPX_V_ETH_USDC>(&pool), expected_shares + MINIMUM_LIQUIDITY);
      assert_eq(interest_protocol_amm::balance_x<ETH, USDC, IPX_V_ETH_USDC>(&pool), eth_amount);
      assert_eq(interest_protocol_amm::balance_y<ETH, USDC, IPX_V_ETH_USDC>(&pool), usdc_amount);
      assert_eq(interest_protocol_amm::decimals_x<ETH, USDC, IPX_V_ETH_USDC>(&pool), ETH_DECIMAL_SCALAR);
      assert_eq(interest_protocol_amm::decimals_y<ETH, USDC, IPX_V_ETH_USDC>(&pool), USDC_DECIMAL_SCALAR);
      assert_eq(interest_protocol_amm::volatile<ETH, USDC, IPX_V_ETH_USDC>(&pool), true);
      assert_eq(interest_protocol_amm::seed_liquidity<ETH, USDC, IPX_V_ETH_USDC>(&pool), MINIMUM_LIQUIDITY);
      assert_eq(interest_protocol_amm::locked<ETH, USDC, IPX_V_ETH_USDC>(&pool), false);
      assert_eq(interest_protocol_amm::admin_balance_x<ETH, USDC, IPX_V_ETH_USDC>(&pool), 0);
      assert_eq(interest_protocol_amm::admin_balance_y<ETH, USDC, IPX_V_ETH_USDC>(&pool), 0);

      let fees = interest_protocol_amm::fees<ETH, USDC, IPX_V_ETH_USDC>(&pool);
    ...
    {      
      ...
      assert_eq(interest_protocol_amm::lp_coin_supply<USDC, USDT, IPX_S_USDC_USDT>(&pool), expected_shares + MINIMUM_LIQUIDITY);
      assert_eq(interest_protocol_amm::balance_x<USDC, USDT, IPX_S_USDC_USDT>(&pool), usdc_amount);
      assert_eq(interest_protocol_amm::balance_y<USDC, USDT, IPX_S_USDC_USDT>(&pool), usdt_amount);
      assert_eq(interest_protocol_amm::decimals_x<USDC, USDT, IPX_S_USDC_USDT>(&pool), USDC_DECIMAL_SCALAR);
      assert_eq(interest_protocol_amm::decimals_y<USDC, USDT, IPX_S_USDC_USDT>(&pool), USDT_DECIMAL_SCALAR);
      assert_eq(interest_protocol_amm::volatile<USDC, USDT, IPX_S_USDC_USDT>(&pool), false);
      assert_eq(interest_protocol_amm::stable<USDC, USDT, IPX_S_USDC_USDT>(&pool), true);
      assert_eq(interest_protocol_amm::seed_liquidity<USDC, USDT, IPX_S_USDC_USDT>(&pool), MINIMUM_LIQUIDITY);
      assert_eq(interest_protocol_amm::locked<USDC, USDT, IPX_S_USDC_USDT>(&pool), false);
      assert_eq(interest_protocol_amm::admin_balance_x<USDC, USDT, IPX_S_USDC_USDT>(&pool), 0);
      assert_eq(interest_protocol_amm::admin_balance_y<USDC, USDT, IPX_S_USDC_USDT>(&pool), 0); 

      let fees = interest_protocol_amm::fees<USDC, USDT, IPX_S_USDC_USDT>(&pool);
  ...
  fun test_volatile_swap() {
    ...
    next_tx(test, alice);
    {
      ...
      assert_eq(interest_protocol_amm::balance_x<ETH, USDC, IPX_V_ETH_USDC>(&pool), eth_amount + amount_in - admin_in_fee);
      assert_eq(interest_protocol_amm::balance_y<ETH, USDC, IPX_V_ETH_USDC>(&pool), usdc_amount - (expected_amount_out + admin_out_fee));
      assert_eq(interest_protocol_amm::admin_balance_x<ETH, USDC, IPX_V_ETH_USDC>(&pool), admin_in_fee);
      assert_eq(interest_protocol_amm::admin_balance_y<ETH, USDC, IPX_V_ETH_USDC>(&pool), admin_out_fee);
    ...
    next_tx(test, alice);
    {
      ...
      let eth_amount = interest_protocol_amm::balance_x<ETH, USDC, IPX_V_ETH_USDC>(&pool);
      let usdc_amount = interest_protocol_amm::balance_y<ETH, USDC, IPX_V_ETH_USDC>(&pool);
      let initial_admin_balance_x = interest_protocol_amm::admin_balance_x<ETH, USDC, IPX_V_ETH_USDC>(&pool);
      let initial_admin_balance_y = interest_protocol_amm::admin_balance_y<ETH, USDC, IPX_V_ETH_USDC>(&pool);
     ...
     let eth_coin = interest_protocol_amm::swap<USDC, ETH, IPX_V_ETH_USDC>(
      ...
      assert_eq(interest_protocol_amm::balance_x<ETH, USDC, IPX_V_ETH_USDC>(&pool), eth_amount - (expected_amount_out + admin_out_fee));
      assert_eq(interest_protocol_amm::balance_y<ETH, USDC, IPX_V_ETH_USDC>(&pool), usdc_amount + amount_in - admin_in_fee);
      assert_eq(interest_protocol_amm::admin_balance_x<ETH, USDC, IPX_V_ETH_USDC>(&pool), admin_out_fee + initial_admin_balance_x);
      assert_eq(interest_protocol_amm::admin_balance_y<ETH, USDC, IPX_V_ETH_USDC>(&pool), admin_in_fee + initial_admin_balance_y);
  ...
  fun test_stable_swap() {
    ...
    next_tx(test, alice);
    {
      ...
      assert_eq(interest_protocol_amm::balance_x<USDC, USDT, IPX_S_USDC_USDT>(&pool), usdc_amount + amount_in - admin_in_fee);
      assert_eq(interest_protocol_amm::balance_y<USDC, USDT, IPX_S_USDC_USDT>(&pool), usdt_amount - (expected_amount_out + admin_out_fee));
      assert_eq(interest_protocol_amm::admin_balance_x<USDC, USDT, IPX_S_USDC_USDT>(&pool), admin_in_fee);
      assert_eq(interest_protocol_amm::admin_balance_y<USDC, USDT, IPX_S_USDC_USDT>(&pool), admin_out_fee);
    ...
    next_tx(test, alice);
    {
      ...
      let usdc_amount = interest_protocol_amm::balance_x<USDC, USDT, IPX_S_USDC_USDT>(&pool);
      let usdt_amount = interest_protocol_amm::balance_y<USDC, USDT, IPX_S_USDC_USDT>(&pool);
      let initial_admin_balance_x = interest_protocol_amm::admin_balance_x<USDC, USDT, IPX_S_USDC_USDT>(&pool);
      let initial_admin_balance_y = interest_protocol_amm::admin_balance_y<USDC, USDT, IPX_S_USDC_USDT>(&pool);
      ...
      assert_eq(burn_for_testing(usdc_coin), expected_amount_out);
      assert_eq(interest_protocol_amm::balance_x<USDC, USDT, IPX_S_USDC_USDT>(&pool), usdc_amount - (expected_amount_out + admin_out_fee));
      assert_eq(interest_protocol_amm::balance_y<USDC, USDT, IPX_S_USDC_USDT>(&pool), usdt_amount + amount_in - admin_in_fee);
      assert_eq(interest_protocol_amm::admin_balance_x<USDC, USDT, IPX_S_USDC_USDT>(&pool), admin_out_fee + initial_admin_balance_x);
      assert_eq(interest_protocol_amm::admin_balance_y<USDC, USDT, IPX_S_USDC_USDT>(&pool), admin_in_fee + initial_admin_balance_y);
  ...
  fun test_add_liquidity() {
    ...
    next_tx(test, alice);
    {
      ...
      let pool_id = interest_protocol_amm::pool_id<Volatile, ETH, USDC>(&registry);
      let pool = test::take_shared_by_id<InterestPool>(test, option::destroy_some(pool_id));
      let initial_lp_coin_supply = interest_protocol_amm::lp_coin_supply<ETH, USDC, IPX_V_ETH_USDC>(&pool);

      let amount_x = 20 * ETH_DECIMAL_SCALAR;
      let amount_y = 27000 * USDC_DECIMAL_SCALAR;

      let (shares, optimal_x, optimal_y) = quote::add_liquidity<ETH, USDC, IPX_V_ETH_USDC>(&pool, amount_x, amount_y);
      ...
      assert_eq(interest_protocol_amm::lp_coin_supply<ETH, USDC, IPX_V_ETH_USDC>(&pool), shares + initial_lp_coin_supply);
      assert_eq(interest_protocol_amm::balance_x<ETH, USDC, IPX_V_ETH_USDC>(&pool), eth_amount + optimal_x);
      assert_eq(interest_protocol_amm::balance_y<ETH, USDC, IPX_V_ETH_USDC>(&pool), usdc_amount + optimal_y);
  ...
  fun test_remove_liquidity() {
    ...
    next_tx(test, alice);
    {
      ...
      let (expected_x, expected_y) = quote::remove_liquidity<ETH, USDC, IPX_V_ETH_USDC>(&pool, initial_lp_coin_supply / 3);
      ...
      assert_eq(interest_protocol_amm::lp_coin_supply<ETH, USDC, IPX_V_ETH_USDC>(&pool), initial_lp_coin_supply - initial_lp_coin_supply / 3);
      assert_eq(interest_protocol_amm::balance_x<ETH, USDC, IPX_V_ETH_USDC>(&pool), eth_amount - expected_x);
      assert_eq(interest_protocol_amm::balance_y<ETH, USDC, IPX_V_ETH_USDC>(&pool), usdc_amount - expected_y);
  ...
  fun test_flash_loan() {
    ...
    next_tx(test, alice);
    {
      ...
      assert_eq(interest_protocol_amm::locked<ETH, USDC, IPX_V_ETH_USDC>(&pool), true);
      assert_eq(interest_protocol_amm::balance_x<ETH, USDC, IPX_V_ETH_USDC>(&pool), eth_amount - eth_coin_amount);
      assert_eq(interest_protocol_amm::balance_y<ETH, USDC, IPX_V_ETH_USDC>(&pool), usdc_amount - usdc_coin_amount);
      ...
      assert_eq(interest_protocol_amm::locked<ETH, USDC, IPX_V_ETH_USDC>(&pool), false);
      assert_eq(interest_protocol_amm::balance_x<ETH, USDC, IPX_V_ETH_USDC>(&pool), eth_amount + invoice_repay_amount_x - eth_coin_amount);
      assert_eq(interest_protocol_amm::balance_y<ETH, USDC, IPX_V_ETH_USDC>(&pool), usdc_amount + invoice_repay_amount_y - usdc_coin_amount);
  ...
  fun test_admin_fees_actions() {
    ...
    next_tx(test, alice);
    {
      ...
      let admin_balance_x = interest_protocol_amm::admin_balance_x<USDC, USDT, IPX_S_USDC_USDT>(&pool);
      let admin_balance_y = interest_protocol_amm::admin_balance_y<USDC, USDT, IPX_S_USDC_USDT>(&pool);
  ...
  fun test_repay_wrong_pool() {
    ...
    next_tx(test, alice);
    {
      let registry = test::take_shared<Registry>(test);
      let pool_id = interest_protocol_amm::pool_id<Volatile, ETH, USDC>(&registry);
      let pool1 = test::take_shared_by_id<InterestPool>(test, option::destroy_some(pool_id));
      let pool_id = interest_protocol_amm::pool_id<Stable, USDC, USDT>(&registry);
      let pool2 = test::take_shared_by_id<InterestPool>(test, option::destroy_some(pool_id));
  ...
  fun test_new_pool_wrong_lp_coin_metadata() {
    ...
    {
      let registry = test::take_shared<Registry>(test);
      let lp_coin_cap = test::take_from_sender<TreasuryCap<IPX_BTCE_ETH>>(test);
      let btc_metadata = test::take_shared<CoinMetadata<BTC>>(test);
      let eth_metadata = test::take_shared<CoinMetadata<ETH>>(test);
      let lp_coin_metadata = test::take_shared<CoinMetadata<IPX_BTCE_ETH>>(test);
  ...
  fun test_add_liquidity_slippage() {
    ...
    next_tx(test, alice);
    {
      ...
      let (shares, _, _) = quote::add_liquidity<ETH, USDC, IPX_V_ETH_USDC>(&pool, amount_x, amount_y);
